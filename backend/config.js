...
app.use(
	cors({
		origin: 'http://localhost:5173', // Allow the frontend running at this port
...
// Define the Transaction model (assuming the table is named `transactions`)
...
app.post('/payment-confirmation', async (req, res) => {
	try {
      ...
      `;
			...
			if (!codes || codes.length === 0) {
				return res.status(404).json({ error: 'No available codes to redeem' });
			}
...
// Helper function to get the plan name from its ID
function getPlanName(planId) {
	switch (planId) {
		case 81:
			return 'Mobile Weekly';
		case 82:
			return 'Mobile Monthly';
		case 83:
